...
import org.helloframework.gateway.common.annotation.Valid;
import org.helloframework.gateway.common.definition.apiservice.plugins.ApiServiceValidatePlugin;
import org.helloframework.gateway.common.definition.apiservice.plugins.ValidateCode;
 ...
 */
@ApiPlugin
public class JSR303ApiServiceValidatePlugin implements ApiServiceValidatePlugin {
    //构建默认的验证器
    private final static Validator validator = Validation.buildDefaultValidatorFactory().getValidator();

    static class JSR303StartBean {}

    static {
        Locale.setDefault(Locale.SIMPLIFIED_CHINESE);
        validator.validate(new JSR303StartBean());
    }

    public List<ValidateMessage> validate(Object o) throws ApiException {
        List<ValidateMessage> validateMessages = new ArrayList<ValidateMessage>();
        if (o == null) {
            validateMessages.add(new ValidateMessage("object", "数据为空", ValidateCode.OBJ_NULL));
        }
        if (!o.getClass().isAnnotationPresent(Valid.class)) {
            return validateMessages;
        }
        Set<ConstraintViolation<Object>> validResult = validator.validate(o);
        if (CollectionUtils.isNotEmpty(validResult)) {
            for (Iterator<ConstraintViolation<Object>> iterator = validResult.iterator(); iterator.hasNext(); ) {
                ...
                validateMessages.add(new ValidateMessage(constraintViolation.getPropertyPath().toString(), StringUtils.isBlank(constraintViolation.getMessage()) ? "不合法" : constraintViolation.getMessage(), ValidateCode.PROPERTY_ILLEGAL));
            }
        }
        return validateMessages;
...
//            apiException.setMsg(validateMessages.get(0).getMessage());
//            throw apiException;
//        }
    }
}
