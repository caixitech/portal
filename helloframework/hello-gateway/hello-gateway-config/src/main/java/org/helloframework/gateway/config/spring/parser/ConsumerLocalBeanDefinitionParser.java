 ...
 * Created by lanjian
 */
public class ConsumerLocalBeanDefinitionParser extends HelloSingleBeanDefinitionParser {
    ...
    protected List<Class> getBeanClass(Element element) {
        try {
            return Arrays.asList(Class.forName(Constants.CLIENT_DEFAULT_LOCAL_CLASS));
        } catch (Exception ex) {
    ...
    protected void doParse(Element element, ParserContext parserContext, BeanDefinitionBuilder builder,Class beanClass) {
        ...
        builder.addPropertyValue(MODE_PROPERTY, Constants.LOCAL_MODE);
