...
public abstract class FailbackRegistry extends AbstractRegistry {
    ...
    // 定时任务执行器
    private final ScheduledExecutorService retryExecutor = Executors.newScheduledThreadPool(1, new NamedThreadFactory("HelloRegistryFailedRetryTimer", true));
...
//                logger.error("Failed to subscribe " + url + ", Using cached list: " + urls + " from cache file: " + getUrl().getParameter(Constants.FILE_KEY, System.getProperty("user.home") + "/hello-registry-" + url.getHost() + ".cache") + ", cause: " + t.getMessage(), t);
//            } else {
//                // 如果开启了启动时检测，则直接抛出异常
//                boolean check = getUrl().getParameter(Constants.CHECK_KEY, true)
//                        && url.getParameter(Constants.CHECK_KEY, true);
